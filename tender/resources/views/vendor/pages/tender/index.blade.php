          ...
          <div class="col-12">
            <div class="card">
              <div class="card-header">
                <h3 class="card-title">List of all tenders available</h3>
              </div>
              <!-- /.card-header -->
              <div class="card-body">
                <table id="example1" class="table table-bordered table-striped">
                  ...
                  <tr>
                    ...
                    <td> {{ $tender->category->category_name }} </td>
                    <td> {{ $tender->tender_title }} </td>
                    <td> {{ $tender->tender_no }} </td>
                    <td> {{ $tender->tender_amount }} </td>
                    <td> {{ date('M d, Y', strtotime($tender->tender_date)) }} </td>
                    <td> {{ date('M d, Y', strtotime($tender->tender_deadline)) }} </td>
                    <td> {{ $tender->tender_status == 1 ? 'Active' : 'Inactive' }} </td>
                    <td><a class="btn btn-info btn-sm" href="{{ url('admin/tender/edit/'.$tender->id) }}"> Edit </a></td>
                  </tr>
                  @endforeach
                </table>
              </div>
